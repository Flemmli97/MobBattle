...
public class MultiItemColor implements IItemColor{
	...
	public int getColor(ItemStack stack, int tintIndex)
    {
		...
		ItemSpawnEgg eggInfo =null;
		if(id!=null)
		{
			eggInfo = ItemSpawnEgg.getEgg(EntityType.getById(id.toString()));
			...
			if(eggInfo==null && CommonProxy.mca)
			{
				...
				{
					int i = stack.getTag().getCompound(ItemExtendedSpawnEgg.tagString).hasKey("GENDER")?stack.getTag().getCompound(ItemExtendedSpawnEgg.tagString).getInteger("GENDER"): 
						stack.getTag().getCompound(ItemExtendedSpawnEgg.tagString).hasKey("MCAGender")?stack.getTag().getCompound(ItemExtendedSpawnEgg.tagString).getInteger("MCAGender"):0;
