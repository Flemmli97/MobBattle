...
public class ItemExtendedSpawnEgg extends Item {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY,
        ...
        if(world.isRemote){
            ...
            /*
             * IBlockState iblockstate = world.getBlockState(pos); Block block = iblockstate.getBlock();
             * 
             * if (block == Blocks.MOB_SPAWNER) { TileEntity tileentity = world.getTileEntity(pos);
             * 
             * if (tileentity instanceof TileEntityMobSpawner) { MobSpawnerBaseLogic mobspawnerbaselogic =
             * ((TileEntityMobSpawner)tileentity).getSpawnerBaseLogic(); mobspawnerbaselogic.setEntityId(getNamedIdFrom(itemstack));
             * mobspawnerbaselogic.getCachedEntity() tileentity.markDirty(); world.notifyBlockUpdate(pos, iblockstate, iblockstate,
             * 3);
             * 
    ...
    public static Entity spawnEntity(World world, ItemStack stack, double x, double y, double z) {
        Entity entity = null;
        if(ItemExtendedSpawnEgg.hasSavedEntity(stack)){
            entity = EntityList.createEntityByIDFromName(new ResourceLocation(stack.getTagCompound().getCompoundTag(tagString).getString("id")),
                    world);
            if(entity instanceof EntityLiving){
                EntityLiving entityliving = (EntityLiving) entity;
                if(CommonProxy.mca && entityliving instanceof EntityVillagerMCA
                        && stack.getTagCompound().getCompoundTag(tagString).hasKey("MCAGender")){
