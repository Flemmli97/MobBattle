...
public class MultiItemColor implements ItemColor {
    ...
    public int getColor(ItemStack stack, int tintIndex) {
        ...
        if (id != null) {
            if (id.equals(Registry.ENTITY_TYPE.getKey(EntityType.WITHER))) {
                return tintIndex == 0 ? 0x161616 : 0x424242;
            }
            if (id.equals(Registry.ENTITY_TYPE.getKey(EntityType.IRON_GOLEM))) {
                return tintIndex == 0 ? 0xd0b096 : 0x658932;
            }
            if (id.equals(Registry.ENTITY_TYPE.getKey(EntityType.SNOW_GOLEM))) {
                return tintIndex == 0 ? 0xffffff : 0xe3901d;
            }
            if (id.equals(Registry.ENTITY_TYPE.getKey(EntityType.GIANT))) {
                id = Registry.ENTITY_TYPE.getKey(EntityType.ZOMBIE);
            }
            if (id.equals(Registry.ENTITY_TYPE.getKey(EntityType.ILLUSIONER))) {
                return tintIndex == 0 ? 0x135893 : 0x848989;
            }
            SpawnEggItem vanillaEgg = SpawnEggItem.byId(Registry.ENTITY_TYPE.get(id));
            if (vanillaEgg != null)
