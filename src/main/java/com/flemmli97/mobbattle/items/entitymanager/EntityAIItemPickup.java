...
public class EntityAIItemPickup extends EntityAIBase
{
    ...
    public boolean shouldExecute()
    {
        ...
        {
            List<EntityItem> list = this.entity.world.<EntityItem>getEntitiesWithinAABB(EntityItem.class, this.entity.getEntityBoundingBox().expandXyz(8));
            if (list.isEmpty())
            {
                return false;
            }
	...
	public void updateTask() {
		for (EntityItem entityitem : this.entity.world.getEntitiesWithinAABB(EntityItem.class, this.entity.getEntityBoundingBox().expand(1.0D, 0.0D, 1.0D)))
        {
	...
	private boolean isItemBetter(ItemStack stack, ItemStack currentEquipped)
	{
		...
		else
		{
			...
			for(AttributeModifier a : currentEquipped.getAttributeModifiers(EntityEquipmentSlot.MAINHAND).get(SharedMonsterAttributes.ATTACK_DAMAGE.getName()))
	...
	protected void updateEquipmentIfNeeded(EntityItem itemEntity)
    {
        ...
        if (this.isItemBetter(itemstack, itemstack1))
        {
            ...
            this.entity.world.playSound(null, entity.posX, entity.posY, entity.posZ, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.PLAYERS, 0.2F, (this.entity.getRNG().nextFloat() - this.entity.getRNG().nextFloat()) * 1.4F + 2.0F);
