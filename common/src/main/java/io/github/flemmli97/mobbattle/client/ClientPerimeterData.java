...
public class ClientPerimeterData {
    ...
    public static void render(Player player, PoseStack stack) {
        ...
        if (player.getMainHandItem().is(MobBattleItems.PERIMETER_TOOL.get()) || player.getOffhandItem().is(MobBattleItems.PERIMETER_TOOL.get())) {
            ...
            ClientHandler.renderShapeOutline(stack, buffer, Shapes.create(aabb), 0.5F, 1, 1, 0.5f);
            ...
            switch (perimeter.shape()) {
                case CIRCLE -> {
                    double pY = Mth.lerp(Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(false),
                            Minecraft.getInstance().player.yOld, Minecraft.getInstance().player.getY());
                    float minY = Minecraft.getInstance().level.getMinBuildHeight();
                ...
                case SQUARE -> {
                    double pY = Mth.lerp(Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(false),
                            Minecraft.getInstance().player.yOld, Minecraft.getInstance().player.getY());
                    AABB perimeterAABB = new AABB(perimeter.center().getX(), Minecraft.getInstance().level.getMinBuildHeight(), perimeter.center().getZ(),
    ...
    private static void renderWall(MultiBufferSource buffer, PoseStack stack, AABB aabb, float red, float green, float blue, float alpha) {
        VertexConsumer consumer = buffer.getBuffer(RenderType.debugStructureQuads());
