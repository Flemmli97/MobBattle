    ...
    public static void applyTeamModSpawnEgg(EntityPlayer player, ItemStack stack) {
        ...
        if(raytraceresult != null && raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK){
            ...
            if(player.world.isBlockModifiable(player, blockpos) && player.canPlayerEdit(blockpos, raytraceresult.sideHit, stack)){
                Entity entity = ItemMonsterPlacer.spawnCreature(player.world, ItemMonsterPlacer.getNamedIdFrom(stack),
                        (double) blockpos.getX() + (liquid ? 0.5D : 0), (double) blockpos.getY() + 0.5D, (double) blockpos.getZ() + 0.5D);
                if(entity != null){
