...
public class Team {
    ...
    {
    		RayTraceResult raytraceresult = Team.rayTrace(player.world, player, true);

        if (raytraceresult != null && raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK)
        {
            BlockPos blockpos = raytraceresult.getBlockPos();
            boolean liquid = player.world.getBlockState(blockpos).getBlock() instanceof BlockLiquid;
            if(!(liquid))
            {
            		blockpos = blockpos.offset(raytraceresult.sideHit);
            		blockpos = new BlockPos (blockpos.getX(), blockpos.getY()+ Team.getYOffset(player.world, blockpos), blockpos.getZ());
            }
            if (player.world.isBlockModifiable(player, blockpos) && player.canPlayerEdit(blockpos, raytraceresult.sideHit, stack))
            {
                Entity entity = ItemMonsterPlacer.spawnCreature(player.world, ItemMonsterPlacer.getNamedIdFrom(stack), (double)blockpos.getX() + (liquid?0.5D:0), (double)blockpos.getY() + 0.5D, (double)blockpos.getZ() + 0.5D);
                if (entity != null)
                {
                    ItemMonsterPlacer.applyItemEntityDataToEntity(player.world, player, stack, entity);
