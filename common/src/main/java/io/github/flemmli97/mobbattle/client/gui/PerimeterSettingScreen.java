...
public class PerimeterSettingScreen extends Screen {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        int xPadding = 16;
        int yOff = xPadding;
        guiGraphics.drawString(this.font, Component.translatable("mobbattle.gui.perimeter.shape"), this.leftPos + xPadding, this.topPos + yOff, CommonColors.WHITE);
        yOff += 16 + 20;
        guiGraphics.drawString(this.font, Component.translatable("mobbattle.gui.perimeter.inner"), this.leftPos + xPadding, this.topPos + yOff, CommonColors.WHITE);
        yOff += 16 + 20;
        guiGraphics.drawString(this.font, Component.translatable("mobbattle.gui.perimeter.outer"), this.leftPos + xPadding, this.topPos + yOff, CommonColors.WHITE);
    ...
    protected void buttons() {
        int padding = 16;
        int yOff = padding + 12;
        this.addRenderableWidget(CycleButton.<PerimeterData.Shape>builder(shape -> Component.translatable(shape.translationKey))
                .withInitialValue(this.perimeter.shape())
        ...
        EditBox innerBox = new EditBox(this.font, this.leftPos + padding, this.topPos + yOff, 36, 10, Component.empty()) {
            ...
            public boolean charTyped(char codePoint, int modifiers) {
                if (Character.isDigit(codePoint) || PerimeterSettingScreen.this.isHelperKey(codePoint)) {
                    if (super.charTyped(codePoint, modifiers) && !this.getValue().isEmpty()) {
                        try {
